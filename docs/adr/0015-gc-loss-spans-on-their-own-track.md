...
destroyed has already handed its totals forward. `lifetime_count = last` and
...
**One span, named for the generations that lost records**, `GC Loss(0,2)`. A record can only
go missing between two reads, so the generations go blind over the same interval and differ
...
- **The track reads as a near-solid bar at default settings**, since gcmon is blind for most of
  every tick. Lower `--rate` or a calmer workload thins it out, and the numbers live in the args
  either way.
- **One extra row per `(pid, iid)`**, on top of the process track, thread track and
  `GC Metrics` group each process already has. Three generations share it, so the row's shape
