...
copies. Measured on any machine the ratio holds at roughly two orders of magnitude — reproduce it
by timing `_remote_debugging.get_gc_stats` against a held `_remote_debugging.GCMonitor` on the same
target. So gcmon was spending upwards of nine tenths of every poll re-deriving what it had derived
...
`GCMonitor` object that attaches once and reads many times. Taking the second is arithmetic. What
it is not is free, because the pid stops being an argument and becomes an identity — something
gcmon now *holds* per process, with a lifetime, in a program whose central hazard is a pid that
outlives the process that owned it ([ADR-0017](0017-monitor-owns-the-pid-lifecycle.md)).

Three things about that turned out to be surprising enough to be worth recording, because each
looks like a defect to a reader who does not know why it is there.
...
while gcmon is attached. Stated precisely, because it is easy to claim more than it gives: the pin
covers exactly the interval gcmon holds the attachment, which is exactly the interval gcmon reads
through it. **So no successful read on Windows can ever be served from a recycled pid.** The pin is
released by the failing read that observes the death, and gcmon performs no further read of that pid
before dropping its cursors — the epoch advance that follows is bookkeeping against state gcmon
already holds, not another look at the process.
...
- **`ValueError` is deliberately outside that translation**, and this is worth spelling out because
  the obvious reading is that it was missed. macOS raises `ValueError` for a read whose arguments
  made no sense *while `task_info` reports the task still valid* — a bad address, not a dead
  process; a terminated macOS target raises `ProcessLookupError`, which is an `OSError` and is
  translated. So `ValueError` means gcmon asked for something wrong, which is a gcmon defect and
  belongs on `PollStatus.FAIL` with a traceback, exactly where it was before this change. Widening
  the translation to swallow it would turn a bug into a silent "target unavailable" and burn the
...
- **Default the reader argument, so existing construction sites keep working.** Rejected. A default
  ...
  to hold the integer it used as a pid — the monitor tests use small integers as pids throughout. A
  required argument fails loudly on a runner instead of silently reading a stranger.

- **Keep the free function and cache nothing.** The status quo, and the thing being replaced. It is
  correct, and it makes gcmon's own cost proportional to the number of processes rather than to the
  number of reads, which is the cost an operator attached to a production process is least willing
  to explain.
...
- **Revalidate the attachment on each read** — compare the target's start time, say. Rejected: it
...
- `tests/test_events_reader.py` covers the lifetime against real subprocesses: a second read of the
  same pid is an order of magnitude cheaper than the first, a failed attach is retried rather than
  remembered, and a failed read is followed by a fresh attach.
- `tests/test_monitor.py` covers what a poll must not do: a target gcmon cannot read yields
  `INVALID_PROCESS` and writes **no** warning, while a failure gcmon does not recognise writes one
  with a traceback. Both halves, because a test watching one passes with the two arms swapped. The
  no-warning half is the regression this decision most invites, since a target exiting is the
  ordinary end of a run.
- `tests/monitoring/test_monitor.py` covers the prune: a pid that leaves the child listing and a pid
  its policy gives up on each lose their attachment in the pass that drops their cursors, and a
  failed child listing drops neither.
