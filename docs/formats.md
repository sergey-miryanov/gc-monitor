...
- **One process track per process**, named `Process 12345` and
  `Process 12345#2` for the second process to hold a PID, each carrying that
  process's own pause rows, `GC Loss` rows, counters, start time and command
  line.
