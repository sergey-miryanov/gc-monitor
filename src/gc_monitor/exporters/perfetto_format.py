...
def build_track_event(
    ...
    result = encode_varint_field(1, type)
    result += encode_varint_field(2, track_uuid)
    if categories:
        for cat in categories:
            result += encode_string_field(3, cat)
    if name is not None:
        result += encode_string_field(4, name)
    if counter_value is not None:
        result += encode_varint_field(5, counter_value)
    if debug_annotations:
        for ann in debug_annotations:
            result += encode_bytes_field(6, ann)
    return result
...
def _make_slice_end(track_uuid: int) -> bytes:
    return build_track_event(type=TYPE_SLICE_END, track_uuid=track_uuid)


def _make_counter_event(track_uuid: int, value: int) -> bytes:
    return build_track_event(type=TYPE_COUNTER, track_uuid=track_uuid, counter_value=value)
