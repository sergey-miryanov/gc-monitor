...
| 0057 | **Landed** 2026-08-22 (efficiency) | S | Every trace gcmon wrote was eight to nine times larger than it had to be, in a format Perfetto already reads compressed. Each batch is deflated into one `TracePacket.compressed_packets` field now, at level 6, with nothing to run before opening the file and no flag to remember. A killed run loses at most the batch it was writing, which is what chose the wrapper over a gzipped file. [ADR-0022](../docs/adr/0022-compress-each-batch-of-packets.md); it took the baseline [0056](0056-intern-the-strings-a-trace-repeats.md) was sized against with it |
