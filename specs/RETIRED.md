...
| 0041 | **Landed** 2026-08-21 (cleanup) | L | Nineteen modules sat in one flat namespace and the layering that held between them was invisible, unchecked and one import from being lost. The package has seven directories now, one per layer, and `tests/test_layering.py` reads the import graph with `ast` and fails when an import crosses a layer the wrong way, naming both modules and the edge. No ADR: the table of allowed edges is in the test, which is where a statement about the architecture can fail. Two questions it left open are settled: the pyperf hook is an entry point rather than a layer, and the deep import paths keep answering through shims for one release, `gcmon.utils` excepted: a shim for it would be a package, which the layer rule does not place, and nothing public named it |
