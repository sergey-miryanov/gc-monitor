...
| 0058 | **Landed** 2026-08-25 (efficiency) | S | gcmon deflated each batch, which cost the monitored process CPU on every flush and still left the file larger than the same events compress to. Each batch goes out as one `TracePacket.zstd_compressed_packets` now, at zstd's own default level, with nothing to run before opening the file and no flag to remember. The cost is a reader older than Perfetto v58: it skips the field, loads a trace with no packets and reports no error, and gcmon cannot warn because it does not know what will open the file, so the minimum version is documented and nothing else. [ADR-0022](../docs/adr/0022-compress-each-batch-of-packets.md) was amended rather than superseded, since the shape it decided, one compressed packet per batch on the flush boundary with no flag, is untouched. The reader keeps field 50 for the suite and for the deflate Perfetto's own tooling writes, and not for an archive: compression had not shipped, so nobody holds a deflated capture |
